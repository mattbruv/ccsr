...
import {
  newGameObjectRenderData,
  newGameMapRenderData,
  getTextureName,
  getMapLocation,
} from "./helpers";
...
class CcsrRenderer {
  ...
  public renderWorld(gameMaps: GameMap[], gameObjects: GameObject[]) {
    ...
    for (const gameMap of gameMaps) {
      ...
      const objectsInMap = gameObjects.filter(
        (x) => x.mapName === gameMap.name
      );
  ...
  private renderGameMap(entry: GameMapRenderData, map: GameMap) {
    ...
    if (map.renderSettings.renderOutOfBounds) {
      entry.mapContainer.mask = null;
