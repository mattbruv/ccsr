...
import { SceneDexter } from "../scenes/dexter";

const maskOverworld = new PIXI.Graphics();
const maskBookRoom = new PIXI.Graphics();

export class Dexter extends EpisodeScript {
  ...
  onNewMap(nextMap: string): void {
    ...
    switch (nextMap) {
      ...
      default: {
        if (
          this.showFinalRoom === false &&
          this.game.inventory.has("painting")
        ) {
  ...
  public init(): void {
    let m: Rect = getMapsRect("0101", "0403");
    ...
    this.game.player.setMapAndPosition(startMap, 10, 10);
    ...
    //   (x) =>
    //     x.mapName === "0301" && x.member == "block.70" && x.location[0] === 1
    // );
    ...
    this.game.addScene("ending", new SceneDexter(this.game));
