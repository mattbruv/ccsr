...
export class SceneDexter extends GameScene {
  ...
  constructor(game: Game) {
    super(game);
    this.screenTexture = new PIXI.Sprite(
      getMemberTexture("ending_1", "ending")
    );
    this.container.addChild(this.screenTexture);

    // add unmasking frames
    const unmaskingFrame = 4 * 12;

    for (let i = 0; i < 14; i++) {
      this.frameCallbacks.push({
        frame: unmaskingFrame + i,
        callback: () => {
          this.screenTexture.texture = getMemberTexture(
            "ending_" + (i + 2),
            "ending"
          )!;
        },
